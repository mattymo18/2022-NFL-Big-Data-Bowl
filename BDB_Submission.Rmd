...
To build the Field Position response variable we first had to normalize the field so in every punting situation the endzone behind the punter was the 0 yardline. From left to right the yardline increases from 0 to 100. We then take the difference between where the ball was kicked and where the ball was at the end of the play. Below we use the gg_field function developed by Marschall Furman to visualize this variable. The function documentation can be found [HERE](https://github.com/mlfurman3/gg_field). 
